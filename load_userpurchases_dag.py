...
class S3ToPostgresTransfer(BaseOperator):
    ...
    def execute(self, context):
        ...
        df_purchase = pd.read_csv(io.StringIO(list_srt_content),
                                  header=0,
                                  delimiter=",",
                                  quotechar='"',
                                  low_memory=False,
        ...
        self.log.info(df_purchase)
        self.log.info(df_purchase.info())

        # formatting and converting the dataframe object in list to prepare the income of the next steps.
        df_purchase = df_purchase.replace(r"[\"]", r"'")
        list_df_purchase = df_purchase.values.tolist()
        list_df_purchase = [tuple(x) for x in list_df_purchase]
        self.log.info(list_df_purchase)
